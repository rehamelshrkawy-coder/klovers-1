...
interface Question { meaning: string; romanization: string; answer: string; options: string[]; }
...
function buildQuestions(vocab: GameVocabItem[]): Question[] {
  ...
  return shuffled.slice(0, 14).map(v => {
    ...
    return { meaning: v.meaning, romanization: v.romanization, answer: v.korean, options: shuffleArray([v.korean, ...wrongs]) };
  });
}

const ReadingChoiceGame = ({ onGameComplete }: { onGameComplete?: (score: number, total: number) => void }) => {
  ...
  if (round >= totalRounds) {
    return (
      <section className="py-12 px-4"><Card className="max-w-lg mx-auto p-8 text-center space-y-4">
        ...
        <h2 className="text-2xl font-bold text-foreground">Reading Choice Complete!</h2>
    ...
    <section className="py-12 px-4">
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <Card className="p-6 text-center space-y-4">
          <p className="text-sm text-muted-foreground">Choose the Korean word for:</p>
          <p className="text-4xl font-bold text-foreground">{q.meaning}</p>
          <p className="text-sm text-muted-foreground italic">{q.romanization}</p>
          <div className="grid grid-cols-2 gap-3" role="group" aria-label="Choose the correct Korean word">
            {q.options.map(opt => (
...
export default ReadingChoiceGame;
