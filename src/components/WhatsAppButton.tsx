...
import { useBottomStripOccupied } from "@/hooks/useBottomSurface";
...
const WhatsAppButton = () => {
  ...
  // A full-width bar owns the bottom strip; the FAB lifts above it instead of
  // sitting on top of the CTA the visitor is reaching for.
  const stripOccupied = useBottomStripOccupied();
  return (
    <button
      type="button"
      onClick={() => trackAndOpenWhatsApp(waUrl, { cta_label: "floating_fab" })}
      aria-label={t("whatsappFab.ariaLabel")}
      className={`group fixed ${stripOccupied ? "bottom-36" : "bottom-24"} transition-[bottom] duration-200 end-5 z-50 flex items-center justify-center w-14 h-14 rounded-full bg-whatsapp shadow-lg hover:bg-whatsapp/90 hover:scale-105 active:scale-95 transition-all duration-200 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:outline-none`}
    >
      ...
      {/* WhatsApp SVG icon */}
      {/* Icon takes the WhatsApp foreground token: white on #25D366 is 1.98:1,
          below the 3:1 floor for a non-text UI element. */}
      <svg viewBox="0 0 32 32" fill="currentColor" className="w-7 h-7 text-whatsapp-foreground" aria-hidden="true">
