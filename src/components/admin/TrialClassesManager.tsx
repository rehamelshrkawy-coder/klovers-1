...
const TrialClassesManager = () => {
  ...
  const handleConfirm = async (booking: TrialBooking, slotValue: string) => {
    ...
    try {
      ...
      // The toast reports what actually happened, including the case where
      // sending is deliberately paused.
      if (emailError) {
        toast({ title: "Confirmed (email failed)", description: `Booking saved. Email error: ${emailError}`, variant: "destructive" });
      } else if (!emailSent) {
        toast({ title: "Confirmed — email skipped (sending is paused)", description: `${booking.name || booking.email} → ${date} at ${time}` });
      } else {
