...
import { PICTURE_VOCAB_WORLDS, getPictureVocabWorldProgress, type PictureVocabWorld } from "@/constants/pictureVocabWorlds";
...
function WorldHeader({ world, progress, isAr }: { world: PictureVocabWorld; progress: { completed: number; total: number; percent: number }; isAr: boolean }) {
  ...
  return (
    <div className="relative flex flex-col items-center" style={{ marginLeft: `${offset}px` }}>
      ...
      >
        ...
        >
          {completed && (
            <div className="absolute -bottom-1 -end-1 bg-primary rounded-full p-0.5">
...
export default function PictureVocabPathMap({ lessons, lessonProgress, userId, bookSlug }: Props) {
  const { language } = useLanguage();
  const isAr = language === "ar";

  return (
    <div className="space-y-8">
      {PICTURE_VOCAB_WORLDS.map((world) => {
        ...
        const progress = getPictureVocabWorldProgress(world, lessonProgress as any, lessons);
